...
public class WorkflowGenerator {
    public let swifts = [
        Swift("swift-50", name: "Swift 5.0", short: "5.0", linux: "swift:5.0", mac: .latest),
        Swift("swift-51", name: "Swift 5.1", short: "5.1", linux: "swift:5.1", mac: .latest),
        Swift("swift-52", name: "Swift 5.2 Nightly", short: "5.2", linux: "swiftlang/swift:nightly-5.2", mac: .toolchain(branch: "swift-5.2-branch")),
        Swift("swift-nightly", name: "Swift Development Nightly", short: "dev", linux: "swiftlang/swift:nightly", mac: .toolchain(branch: "development")),
    ]
    ...
    func enabledSwifts(for repo: Repo) -> [Swift] {
    ...
    public func enabledIdentifiers(for options: [Option], toggleSet toggles: [Bool]) -> [String] {
     ...
     func generateYAML(for repo: Repo, platforms: [Platform], swiftVersions: [Swift], application: BundleInfo) -> String {
         ...
         for platform in platforms {
             source.append(platform.yaml(repo: repo, swifts: swiftVersions, configurations: enabledConfigs(for: repo)))
         }

         return source
     }
     
     func generateHeader(for repo: Repo, platforms: [Platform], swiftVersions: [Swift], application: BundleInfo) -> (String, String) {
         var header = ""
         ...
         if repo.settings.header {
             ...
             let platformIDs = platforms.map({ $0.id })
             let swiftBadges = swiftVersions.map({ "![swift \($0.short) shield]" }).joined(separator: " ")
             let swiftShields = swiftVersions.map({ "[swift \($0.short) shield]: \(repo.imgShieldURL(for: $0)) \"Swift \($0.short)\"" }).joined(separator: "\n")
    ...
    public func generateWorkflow(for repo: Repo, application: BundleInfo) -> Workflow? {
        ...
        let source = generateYAML(for: repo, platforms: platforms, swiftVersions: swiftVersions, application: application)
        let (header, delimiter) = generateHeader(for: repo, platforms: platforms, swiftVersions: swiftVersions, application: application)
